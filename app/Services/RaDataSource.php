...
class RaDataSource
{
	...
	public function __construct(
		...
		$this->sessions = $database->table("sessions");
	}
	...
	public function checkSession($sessionId, $sessionHash)
	{
		...
		$sess_my = $this->sessions->get((int)$sessionId);
		$session = $this->database->fetch('SELECT hash, device_id, started, session_key FROM sessions WHERE id = ?', (int)$sessionId);
		if ($session === null) {
			$all .= ($sess_my === null) ? " sess_my is null..." : " sess_my->id = " . $sess_my->id;
			throw new \App\Exceptions\NoSessionException("(fu: checkSession) - session " . (int)$sessionId . " not found. AS:" . $all);
		}
