...
class InventoryDataSource
{
	...
	public function getDevicesUser($userId): Devices
	{
		...
		foreach ($result as $row) {
			$dev = new Device($row);
			//$dev->attrs['problem_mark'] = false; // @deprecadet use $dev->problem_mark
			$dev->problem_mark = false;
			...
			if ($dev->attrs['last_bad_login'] != NULL) {
				if ($dev->attrs['last_login'] != NULL) {
					...
					if ($lastErrLoginTs >  $lastLoginTs) {
						//$dev->attrs['problem_mark'] = true; // @deprecadet use $dev->problem_mark
